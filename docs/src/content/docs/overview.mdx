...
3. Run `boltffi pack apple`, `boltffi pack android`, `boltffi pack java`, `boltffi pack csharp`, or `boltffi pack wasm`. `pack all` builds every enabled target.
4. Import or include the generated bindings in your project

The generated bindings use native idioms. Swift gets `async`/`await` and throwing functions. Kotlin gets coroutines and sealed classes. Java gets CompletableFuture and records (Java 16+) or plain classes (Java 8+). C# gets `Task`, records/structs, `IDisposable` wrappers, and `await foreach` streams. TypeScript gets Promises and typed interfaces. Your Rust library feels like a native SDK.
...
BoltFFI has full support for Swift, Kotlin, Java, C#, and TypeScript (WASM). Adding support for more languages is in progress.
